...
import SpecialDiscountFormDialog from './form-dialog';

const routes = {
    index: (query) => route('setting.special-discount.index', query ? { query } : undefined),
    store: route('setting.special-discount.store'),
    update: (id) => route('setting.special-discount.update', { specialDiscount: id }),
    destroy: (id) => route('setting.special-discount.destroy', { specialDiscount: id }),
};
const PERM = 'setting.special-discount';

export default function SpecialDiscountIndex({ specialDiscounts, filters, discountTypes }) {
    ...
    const columns = [
        {
            id: 'num',
            header: '#',
            render: (_, i) => (specialDiscounts.from ?? 0) + i,
        },
        { header: 'Name', accessorKey: 'name' },
        { header: 'Amount Range', accessorKey: 'amount_label' },
        { header: 'Discount', accessorKey: 'discount_label' },
        {
            id: 'status',
            header: 'Status',
            render: (row) => (
            ...
            <div className="px-2 py-1">
                <div className="mb-3 flex items-center justify-between rounded-lg bg-blue-950 px-5 py-3">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h1 className="text-base font-semibold text-white">Special Discounts</h1>
                            <p className="text-xs text-white/60">Auto-apply discounts when sale amount reaches a threshold.</p>
                        </div>
                    </div>
                    <AdminCreateButton
                ...
                <DataTable columns={columns} rows={specialDiscounts.data} rowKey="id" emptyMessage="No special discounts found." />

                {specialDiscounts.links?.length > 3 && (
                    <div className="mt-4 flex flex-wrap gap-1">
                        {specialDiscounts.links.map((link, i) => (
                            <Link
                                key={i}
                                href={link.url ?? '#'}
                                className={[
                ...
                <Dialog open={!!deleting} onOpenChange={(open) => !open && setDeleting(null)}>
                    <DialogContent className="p-0">
                        <div className="flex items-center gap-2.5 bg-blue-950 px-5 py-3">
                            ...
                            <h2 className="text-sm font-semibold text-white">Delete Special Discount</h2>
                        </div>
