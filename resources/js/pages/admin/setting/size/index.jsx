...
const routes = settingRoutes('size');
const PERM = 'setting.size';

export default function SizeIndex({ sizes, filters }) {
    ...
    return (
        <>
            ...
            <div className="px-2 py-1">
                <div className="mb-3 flex items-center justify-between rounded-lg bg-blue-950 px-5 py-3">
                    <div className="flex items-center gap-3">
                        <div className="flex size-8 items-center justify-center rounded-md bg-white/15">
                            <Ruler className="size-4 text-white" />
                        </div>
                        <div>
                            <h1 className="text-base font-semibold text-white">Sizes</h1>
                            <p className="text-xs text-white/60">Manage product size variants.</p>
                        </div>
                    </div>
                ...
                <div className="mb-4 flex flex-wrap items-end gap-2">
                    ...
                    <ListDateExportBar
                        ...
                        exportExcelRoute="setting.size.export-excel"
                        exportPdfRoute="setting.size.export-pdf"
                        exportPrintRoute="setting.size.export-print"
                        query={{ search }}
                    />
                </div>

                <DataTable columns={columns} rows={sizes.data} rowKey="id" emptyMessage="No sizes found." />

                {sizes.links?.length > 3 && (
                    <div className="mt-4 flex flex-wrap gap-1">
                        {sizes.links.map((link, i) => (
                            <Link
                                key={i}
                                href={link.url ?? '#'}
                                className={[
            ...
            <Dialog open={!!deleting} onOpenChange={(open) => !open && setDeleting(null)}>
                <DialogContent className="p-0">
                    <div className="flex items-center gap-2.5 bg-blue-950 px-5 py-3">
                        ...
                        <h2 className="text-sm font-semibold text-white">Delete Size</h2>
                    </div>
            ...
            <SettingFormDialog
                ...
                routes={routes}
                fields={[
                    { name: 'name', label: 'Name', placeholder: 'Size name', required: true },
