...
export default function DamageCreate({ today }) {
    const { flash } = usePage().props;
    const toast = useAppToast();
    const form = useForm({ date: today, comment: '', items: [] });
    ...
    function handleSubmit(e) {
        ...
        form.post(route('inventory.damage.store'), {
            preserveScroll: true,
            onError: (errors) => {
    ...
    return (
        <>
            <Head title="New Damage" />
            <div className="px-2 py-1">
                <InventoryPageHeader
                    title="Record Damage"
                    subtitle="Deduct damaged stock from inventory."
                    icon={AlertTriangle}
