...
export default function StockDistributionEdit({ distribution, branches = [], today }) {
    const { flash } = usePage().props;
    const toast = useAppToast();
    const [items, setItems] = useState(distribution.items ?? []);
    ...
    function lineMaxQty(item) {
        return parseFloat(item.max_quantity ?? item.available_stock ?? 0);
    }

    function addItem(item) {
        const mainStock = parseFloat(item.available_stock ?? 0);
        if (mainStock <= 0) {
        ...
        if (exists) {
            setItems((prev) =>
                prev.map((it) => {
                    ...
                    const nextQty = Math.min(parseFloat(it.quantity) + 1, lineMaxQty(it));
    ...
    function handleSubmit(e) {
        ...
        form.put(route('inventory.stock-distribution.update', distribution.id), {
            preserveScroll: true,
            onError: (errors) => {
    ...
    return (
        <>
            <Head title={`Edit Distribution — ${distribution.invoice_number}`} />
            <div className="px-2 py-1">
                <InventoryPageHeader
                    title="Edit Distribution"
                    subtitle={distribution.invoice_number}
                    icon={ArrowRightLeft}
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                            ...
                            <div className="mt-4">
                                ...
                                >
                                    {items.map((item, i) => {
                                        ...
                                        const maxQty = lineMaxQty(item);
                                        const overStock = qty > maxQty;
                    ...
                    <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                        ...
                        />
                        <InventoryCard title="Accounting" icon={Building2}>
                            <p className="text-xs text-muted-foreground">
                                Updating will reverse the previous journal entry, restore and re-transfer stock,
                                then post a new Branch Inventory / Main Inventory entry.
                            </p>
                        </InventoryCard>
                    </div>
                    ...
                    <InventoryFormActions
                        ...
                        submitLabel="Update Distribution"
