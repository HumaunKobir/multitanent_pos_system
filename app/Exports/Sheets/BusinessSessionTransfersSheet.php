...
class BusinessSessionTransfersSheet implements FromCollection, ShouldAutoSize, WithEvents, WithHeadings, WithTitle
{
    private const COLUMN_COUNT = 9;

    /** @var list<int> */
    private const CURRENCY_COLUMNS = [6];
    ...
    public function collection(): Collection
    {
        $rows = collect($this->rows)->map(fn (array $row) => [
            BusinessSessionSheetStyles::formatDisplayDate($row['date'] ?? null),
            $row['time'] ?? '',
            $row['reference'] ?? '',
            $row['from_account'] ?? '',
            $row['to_account'] ?? '',
