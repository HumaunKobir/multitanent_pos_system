...
export default function PurchaseReturnIndex({ returns = { data: [] }, filters = {} }) {
    ...
    useDebouncedEffect(
        () =>
            router.get(
                route('inventory.purchase-return.index'),
                {
    ...
    const columns = [
        ...
        { id: 'date', header: 'Date', render: (row) => formatBdDate(row.date) },
        {
            ...
            header: 'Supplier',
            render: (row) => {
                const company = row.supplier?.company_name?.trim();
                const person = row.supplier?.name?.trim();

                return company && person ? `${company} (${person})` : company || person || '—';
            },
        },
            ...
            <div className="px-2 py-1">
                ...
                <div className="mb-4 flex flex-wrap items-end gap-2">
                    ...
                    <ListDateExportBar
                        ...
                        exportExcelRoute="inventory.purchase-return.export-excel"
                        exportPdfRoute="inventory.purchase-return.export-pdf"
                        exportPrintRoute="inventory.purchase-return.export-print"
                        query={{ search }}
                    />
                </div>
