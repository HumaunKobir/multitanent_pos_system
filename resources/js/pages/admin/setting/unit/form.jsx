...
export default function UnitForm({ unit }) {
    const isEditing = !!unit;

    const form = useForm({
        name: unit?.name ?? '',
        status: unit ? String(unit.status) : '1',
    });

    function handleSubmit(e) {
        e.preventDefault();
        if (isEditing) {
            form.patch(UnitController.update.url(unit.id));
        } else {
    ...
    return (
        <>
            ...
            <div className="p-6">
                <div className="mb-5">
                    <h1 className="text-xl font-semibold">{isEditing ? 'Edit Unit' : 'Create Unit'}</h1>
                </div>
