import{n as e,r as t,t as n}from"./format-bd-date-D8J8Ea5-.js";import{n as r}from"./pos-discount-ufFaP-KB.js";var i=`
* {
...
`;function a(e){return String(e??``).replace(/&/g,`&amp;`).replace(/</g,`&lt;`).replace(/>/g,`&gt;`).replace(/"/g,`&quot;`).replace(/'/g,`&#39;`)}function o(e){if(!e)return``;try{return new URL(e,window.location.href).href}catch{return String(e)}}async function s(e){let t=o(e);if(!t)return``;if(t.startsWith(`data:`))return t;try{let e=await fetch(t,{credentials:`same-origin`});if(!e.ok)return t;let n=await e.blob();return await new Promise(e=>{let r=new FileReader;r.onloadend=()=>e(String(r.result||t)),r.onerror=()=>e(t),r.readAsDataURL(n)})}catch{return t}}async function c(e){let t=e?.order?e:C(e?.sell??{},e?.options??e);return t.options.companyLogo&&(t.options.companyLogo=await s(t.options.companyLogo)),t}function l(e,t){let n=Array.from(e.document.images??[]);if(n.length===0){t();return}let r=n.length,i=()=>{--r,r<=0&&t()};for(let e of n){if(e.complete){i();continue}e.addEventListener(`load`,i,{once:!0}),e.addEventListener(`error`,i,{once:!0})}}function u(e){return`${parseFloat(e??0).toLocaleString(`en-BD`,{minimumFractionDigits:2,maximumFractionDigits:2})} TK`}function d(e){let t=parseFloat(e??0);return Number.isInteger(t)?String(t):t.toFixed(2)}function f(e,t=28){let n=String(e??``).trim();return n.length<=t?n:`${n.slice(0,t-1)}…`}function p(e){return n(e).replace(`, `,` `)}function m(e){return t(e)}function h(){return e(new Date)}function g(e){let t=new Map;for(let n of e){let e=`${n.is_free_row?`free`:`paid`}-${n.variant_id??n.variation_id??`product-${n.product_id??n.id}`}`,r=t.get(e);if(r){r.quantity+=parseFloat(n.quantity??0),r.amount+=parseFloat(n.amount??0),r.line_discount=parseFloat(r.line_discount??0)+parseFloat(n.line_discount??0),r.promotion_discount=parseFloat(r.promotion_discount??0)+parseFloat(n.promotion_discount??0);continue}t.set(e,{...n,quantity:parseFloat(n.quantity??0),amount:parseFloat(n.amount??0),line_discount:parseFloat(n.line_discount??0),promotion_discount:parseFloat(n.promotion_discount??0)})}return Array.from(t.values())}function _(e){let t=e.variant_id??e.variation_id??null;if(t==null||t===``)return``;let n=(e.variant?.name??e.variant_label??e.variation?.variation_data?.label??``).trim(),r=(e.variant?.sku??e.variant_sku??e.variation?.sku_code??``).trim();return n||r}function v(e){let t=f(e.product?.name??e.name??`—`),n=_(e),r=n?`${t} (Variant: ${n})`:t;return e.is_free_row?`${r} (FREE)`:r}function y(e,t){return`
        ...
        <div class="pos-header">
            ...
            <div class="pos-small">Date: ${a(p(n.date))} | Time: ${a(m(n.created_at))}</div>
        ...
        <div class="pos-items">
            ...
            <div class="pos-divider"></div>
            ${o.map(e=>{let t=[];return e.promotion_label&&t.push(`Promo: ${e.promotion_label}`),parseFloat(e.promotion_discount??0)>0&&t.push(`Promo Disc: ${u(e.promotion_discount)}`),parseFloat(e.line_discount??0)>0&&t.push(`Disc: ${u(e.line_discount)}`),`
                <div class="pos-item">
                    <div class="pos-item-name">${a(v(e))}</div>
                    <div class="pos-item-qty">${d(e.quantity)}</div>
                    <div class="pos-item-price">${u(e.amount??e.price)}</div>
                </div>
                ${t.length?`<div class="pos-item-meta">${a(t.join(` | `))}</div>`:``}`}).join(``)}
        </div>`:``,f=x(n.payments),_=f.length>0,b=[r.gross==null?``:y(`Subtotal`,r.gross),parseFloat(r.lineDiscount??0)>0?y(`Line Discount`,r.lineDiscount):``,parseFloat(r.promotionDiscount??0)>0?y(`Promotion Discount`,r.promotionDiscount):``,parseFloat(r.invoiceDiscount??0)>0?y(`Invoice Discount`,r.invoiceDiscount):``,parseFloat(r.specialDiscount??0)>0?y(r.specialDiscountName?`Special (${r.specialDiscountName})`:`Special Discount`,r.specialDiscount):``,parseFloat(r.coinDiscount??0)>0?y(`Coin Discount`,r.coinDiscount):``,parseFloat(r.roundOff??0)>0?y(`Round Off`,r.roundOff):``,parseFloat(r.vat??0)>0?y(`VAT`,r.vat):``,y(`Total`,r.net??n.total_price),y(`Paid`,r.paid??n.paid),parseFloat(r.due??n.due??0)>0?y(`Due`,r.due??n.due):``,parseFloat(r.change??n.change??0)>0?y(`Change`,r.change??n.change):``].filter(Boolean).join(``),C=t.showTotals?`
        <div class="pos-totals">
            ...
            ${f}`:``}
        </div>`:``,w=n.comment?`<div class="pos-note"><span class="pos-bold">Note:</span> ${a(n.comment)}</div>`:``,T=S(t.termsAndConditions)?`<div class="pos-terms"><div class="pos-terms-title">Terms & Conditions</div>${t.termsAndConditions}</div>`:``,E=t.showFooter?`
        <div class="pos-footer">
            ...
            <div class="pos-divider"></div>
            <div class="pos-tiny">Printed on: ${a(h())}</div>
...
</html>`}async function T(e){try{let t=w(await c(e)),n=window.open(``,`_blank`,`width=320,height=700`);if(!n){alert(`Please allow popups to print the POS receipt.`);return}n.document.open(),n.document.write(t),n.document.close();let r=!1,i=()=>{r||n.closed||(r=!0,n.focus(),n.print(),n.onafterprint=()=>n.close())},a=()=>{l(n,()=>{requestAnimationFrame(()=>i())})};n.onload=a,setTimeout(a,300)}catch(e){console.error(`POS print failed:`,e),alert(`Failed to print POS receipt.`)}}typeof window<`u`&&(window.printPos=(e,t={})=>{T(e?.order?e:C(e,t))},window.printPosInvoice=window.printPos,window.PosPrint={print:window.printPos,printInvoice:window.printPos,buildFromSell:C});export{S as n,T as r,C as t};